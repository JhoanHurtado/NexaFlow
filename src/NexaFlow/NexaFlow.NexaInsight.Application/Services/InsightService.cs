...
public class InsightService : IInsightService
{
    ...
    public async Task<AverageTicketDto> GetAverageTicketAsync(Guid tenantId, DateOnly from, DateOnly to)
    {
        ...
        return new AverageTicketDto(result.TenantId, result.Average, result.Total,
            result.SaleCount, from.ToString(), to.ToString());
    }

    public async Task<CancellationRateDto> GetCancellationRateAsync(Guid tenantId, DateOnly from, DateOnly to)
    {
        ValidateRange(from, to);
        ...
        return new CancellationRateDto(result.TenantId, result.TotalReservations,
            result.CancelledReservations, result.RatePercent, from.ToString(), to.ToString());
    }

    public async Task<IEnumerable<DailySummaryDto>> GetDailySummaryAsync(Guid tenantId, DateOnly from, DateOnly to)
    {
        ValidateRange(from, to);
        ...
        return rows.Select(r => new DailySummaryDto(r.Date, r.TotalRevenue, r.SaleCount, r.AverageTicket));
    }
